...
jsurvSex <- glm(SurvivedNext~Sex,data=juv,family = binomial,na.action=na.exclude)
...
We then tested whether maternal condition was passed on to offspring by testing for a relaitonship between maternal and offspring telomere length and dynamics. Offspring TL was positively related to maternal adult TL (estimate = `r getstat(juvTL_LmumTL, 'LmumTL','est')`, CIs = `r getstat(juvTL_LmumTL, 'LmumTL','CI')`; Fig. 2A). A linear regression showed that maternal telomere length explains a modest amount of variation in offspring telomere length (R^2^ = `r getstat(juvmumTLR, 'LmumTL','R2')`), with a heritability of `r round2(2*as.numeric(getstat(juvmumTLR, 'LmumTL','est',standardise = F)))`. Paternal telomere length was not related to offspring telomere length (estimate = `r getstat(juvTL_LdadTL, 'LdadTL','est')`, CIs = `r getstat(juvTL_LdadTL, 'LdadTL','CI')`).

Finally, we tested how maternal telomere length 
