...
jsurv <- glmer(SurvivedNext~SexEstimate+LmumTL+TL+(1|LayYear),data=juv,family = binomial,na.action=na.exclude)
jsurvSex <- glmer(SurvivedNext~SexEstimate + (1|mother) + (1|LayYear),data=juv,family = binomial,na.action=na.exclude)
jsurvTL <- glmer(SurvivedNext~LmumTL+(1|mother) + (1|LayYear),data=juv,family = binomial,na.action=na.exclude)
...
Finally, we tested how maternal telomere length was related to juvenile survival. Considered seperately, neither maternal telomere length nor sex were related to juvenile survival (maternal telomere length: est = `r getstat(jsurvTL, 'LmumTL', 'est')`; CI = `r getstat(jsurvTL, 'LmumTL', 'CI')`; sex: est = `r getstat(jsurvSex, 'SexEstimate', 'est')`; CI = `r getstat(jsurvSex, 'SexEstimate', 'CI')`. However, when maternal telomere length and offspring sex were included together in a model, there was a significant difference in survival between male and female offspring (est = `r getstat(jsurv, 'SexEstimate', 'est')`; CI = `r getstat(jsurv, 'SexEstimate', 'CI')`), with a higher survival rate in male offspring (Fig. 2B). The main effect  of maternal telomere length on offspring survival remained non-significant when sex was included in the model (est = `r getstat(jsurv, 'LmumTL', 'est')`; CI = `r getstat(jsurv, 'LmumTL', 'CI')`).
