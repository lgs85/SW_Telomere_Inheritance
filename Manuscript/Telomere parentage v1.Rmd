...
sr_EdadTL <- glmer(SexEstimate ~ EdadTL + Ageclass + TQ*Helper + (1|father),data = juv,family = 'binomial', na.action=na.exclude)
sr_EmumTL <- lmer(SexEstimate ~ EmumTL + Ageclass + TQ*Helper + (1|mother),data = juv,family = 'binomial',na.action=na.exclude)

#Sex ratio and parental juvenle TL
sr_LparTL <- glmer(SexEstimate ~ LdadTL + LmumTL + Ageclass + TQ*Helper + (1|mother) + (1|father),data = juv,family = 'binomial',na.action=na.exclude)

#Sex ratio and parental age
sr_parage <- glmer(SexEstimate ~ mumage + dadage + Ageclass + TQ*Helper + (1|mother) + (1|father),data = juv,family = 'binomial',na.action=na.exclude)

#Sex ratio and parental lifespan
sr_parlife <- glmer(SexEstimate ~ mumlife + dadlife + Ageclass + TQ*Helper + (1|mother) + (1|father),data = juv,family = 'binomial',na.action=na.exclude)


```


Seychelles warbler juvenile TL was not related to maternal juvenile TL (estimate = `r getstat(juvTL_EmumTL, 'EmumTL','est')`, CIs = `r getstat(juvTL_EmumTL, 'EmumTL','CI')`; Fig. 1A), but was positively related to maternal adult TL (estimate = `r getstat(juvTL_LparTL, 'LmumTL','est')`, CIs = `r getstat(juvTL_LparTL, 'LmumTL','CI')`; Fig. 1B). Conversely, juvenile TL was positively related to paternal juvenile TL (estimate = `r getstat(juvTL_EdadTL, 'EdadTL','est')`, CIs = `r getstat(juvTL_EdadTL, 'EdadTL','CI')`; Fig. 1C), but was not related to paternal adult TL (estimate = `r getstat(juvTL_LparTL, 'LdadTL','est')`, CIs = `r getstat(juvTL_LparTL, 'LdadTL','CI')`; Fig. 1D). Juvenile TL was not related to maternal age (estimate = `r getstat(juvTL_parage, 'mumage','est')`, CIs = `r getstat(juvTL_parage, 'mumage','CI')`; Fig. 2A), but was positvely associated with maternal lifespan (estimate = `r getstat(juvTL_parlife, 'mumlife','est')`, CIs = `r getstat(juvTL_parlife, 'mumlife','CI')`; Fig. 2B). Finally, juvenile TL was not associated with paternal age (estimate = `r getstat(juvTL_parage, 'dadage','est')`, CIs = `r getstat(juvTL_parage, 'dadage','CI')`; Fig. 2C) or paternal lifespan (estimate = `r getstat(juvTL_parlife, 'dadlife','est')`, CIs = `r getstat(juvTL_parlife, 'dadlife','CI')`; Fig. 2D).

Offspring sex ratio was not related to maternal juvenile TL (estimate = `r getstat(sr_EmumTL, 'EmumTL','est')`, CIs = `r getstat(sr_EmumTL, 'EmumTL','CI')`; Fig. 3A), but longer maternal adult TL was significantly related to offspring sex ratio (estimate = `r getstat(sr_LparTL, 'LmumTL','est')`, CIs = `r getstat(sr_LparTL, 'LmumTL','CI')`); mothers with longer telomeres as adults had more females offspring compared to mothers with shorter telomeres (Fig. 3B). A similar result was found for patenral TL: offspring sex ratio was not related to paternal juvunile TL (estimate = `r getstat(sr_EdadTL, 'EdadTL','est')`, CIs = `r getstat(sr_EdadTL, 'EdadTL','CI')`; Fig. 3C), but fathers with longer telomeres as adults had significantly more females (estimate = `r getstat(sr_LparTL, 'LdadTL','est')`, CIs = `r getstat(sr_LparTL, 'LdadTL','CI')`; Fig. 3D). 

Offspring sex ratio was not related to either maternal age (estimate = `r getstat(sr_parage, 'mumage','est')`, CIs = `r getstat(sr_parage, 'mumage','CI')`; Fig. 4A) or maternal lifespan (estimate = `r getstat(sr_parlife, 'mumlife','est')`, CIs = `r getstat(sr_parlife, 'mumlife','CI')`; Fig. 4B). However, older fathers had significantly more female offspring (estimate = `r getstat(sr_parage, 'dadage','est')`, CIs = `r getstat(sr_parage, 'dadage','CI')`; Fig. 4C). There was no relationship between paternal lifespan and offspring sex ratio (estimate = `r getstat(sr_parlife, 'dadlife','est')`, CIs = `r getstat(sr_parlife, 'dadlife','CI')`; Fig. 4D).

To tease apart whether parental age or telomere effects were most importand driving offspring telomere length, we used an information theoretic approach. The best model with offspring telomere length as the response variable was the one containing parental adult telomere lengths as predictors (AICc = `r round2(AICc(juvTL_LparTL))`), which was a better fit than the model containing parental age (AICc = `r round2(AICc(juvTL_parage))`) and parental lifespan (AICc = `r round2(AICc(juvTL_parlife))`). The same result was found when offspring sex raito was the response variable: the model with parental telomere length as predictors had a lower AICc (`r round2(AICc(sr_LparTL))`) than models containing parental age (AICc = `r round2(AICc(sr_parage))`) or parental lifespan (AICc = `r round2(AICc(sr_parlife))`).
