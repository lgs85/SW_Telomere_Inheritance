...
lm2 <- lmer(TL~EmumTL + (1|mother) + (1|father) + (1|FieldPeriodID),data = juv,na.action=na.exclude)
lm2 <- standardize(lm2)
summary(lm2)

lm2 <- lmer(TL~EdadTL + (1|mother) + (1|father) + (1|FieldPeriodID),data = juv,na.action=na.exclude)
lm2 <- standardize(lm2)
summary(lm2)

lm2 <- lmer(TL~LmumTL + (1|mother) + (1|father) + (1|FieldPeriodID),data = juv,na.action=na.exclude)
lm2 <- standardize(lm2)
summary(lm2)

lm2 <- lmer(TL~LdadTL + (1|mother) + (1|father) + (1|FieldPeriodID),data = juv,na.action=na.exclude)
lm2 <- standardize(lm2)
summary(lm2)
...
Fig3A <- ggplot(subset(juv,EPP == 'Within pair'),aes(x = mumlife,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
...
Fig3C <- ggplot(subset(juv,EPP == 'Within pair'),aes(x = mumage,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
...
lm2 <- glmer(SexEstimate~LdadTL + (1|father)+ (1|mother),data=juv,family='binomial',na.action=na.exclude)
lm2 <- standardize(lm2)
summary(lm2)
