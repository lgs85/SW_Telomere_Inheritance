...
juvTL_LmumTLSex <- lmer(TL~LmumTL*Sex + Fledged + Tarsus + Helper +(1|mother) + (1|LayYear),data = juv2,na.action=na.exclude)
...
Generalised linear mixed models controlling for maternal age, territory quality and availability of helpers showed that offspring sex ratio was significantly related to maternal adult telomere length (estimate = `r getstat(sr_LmumTL, 'LmumTL','est',standardise = F)`, CIs = `r getstat(sr_LmumTL, 'LmumTL','CI',standardise = F)`; Table S1). Mothers with short telomeres (<4kb) as adults produced on `r round(mean(subset(juv,LmumTL<4000)$SexEstimate)*100,0)`% males, while mothers with long telomeres (>7kb) produced `r 100-round(mean(subset(juv,LmumTL>7000)$SexEstimate)*100,0)`% females (Fig. 1B).We found no evidence that sex ratio was linked to paternal telomere length (estimate = `r getstat(sr_LdadTL, 'LdadTL','est',standardise = F)`, CIs = `r getstat(sr_LdadTL, 'LdadTL','CI',standardise = F)`). We also found no effect of territory quality or the number of helpers on offspring sex ratio (Table S1).

We then tested whether maternal condition was passed on to offspring by testing for a relationship between maternal and offspring telomere length and dynamics. Offspring TL was positively related to maternal TL (estimate = `r getstat(juvTL_LmumTL, 'LmumTL','est',standardise = F)`, CIs = `r getstat(juvTL_LmumTL, 'LmumTL','CI',standardise = F)`), and this effect did not vary with offspring sex (estimate = `r getstat(juvTL_LmumTLSex, 'LmumTL:SexMales','est',standardise = F)`, CIs = `r getstat(juvTL_LmumTLSex, 'LmumTL:SexMales','CI',standardise = F)` ; Fig. 2A). A linear regression showed that maternal telomere length explains a modest amount of variation in offspring telomere length (R^2^ = `r getstat(juvmumTLR, 'LmumTL','R2',standardise = F)`), with a heritability of `r round2(2*as.numeric(getstat(juvmumTLR, 'LmumTL','est',standardise = F)))`. Paternal telomere length was not related to offspring telomere length (estimate = `r getstat(juvTL_LdadTL, 'LdadTL','est',standardise = F)`, CIs = `r getstat(juvTL_LdadTL, 'LdadTL','CI',standardise = F)`).

Finally, we tested how juvenile telomere length in each was related to survival. Considered seperately, neither telomere length nor sex were related to juvenile survival to adulthood (telomere length: est = `r getstat(jsurvTL, 'TL', 'est',standardise = F)`; CI = `r getstat(jsurvTL, 'TL', 'CI',standardise = F)`; sex: est = `r getstat(jsurvSex, 'SexEstimate', 'est',standardise = F)`; CI = `r getstat(jsurvSex, 'SexEstimate', 'CI',standardise = F)`. However, when telomere length and  sex were included together in a model, the effects of both variables increased notably (telomere length: est = `r getstat(jsurv, 'TL', 'est',standardise = F)`; CI = `r getstat(jsurv, 'TL', 'CI',standardise = F)`; sex: est = `r getstat(jsurv, 'SexEstimate', 'est',standardise = F)`; CI = `r getstat(jsurv, 'SexEstimate', 'CI',standardise = F)`), with the sex effect becoming significant. Interestingly, there was also a significant interaction effect between sex and telomere length on survival (est = `r getstat(jsurv, 'SexEstimate:TL', 'est',standardise = F)`; CI = `r getstat(jsurv, 'SexEstimate:TL', 'CI',standardise = F)`). Telomere length iin female juveniles was positively related to juvenile survival, while no such relationship was observed in males (Fig. 2B).
