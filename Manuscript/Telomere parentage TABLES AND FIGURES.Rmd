...
Fig2A <- ggplot(juv,aes(x = LdadTL,y = LmumTL)) +
...
Fig2B <- ggplot(juv,aes(x = LmumTL,y = SexEstimate)) +
  geom_point(alpha=0.2) +
...
Fig2C <- ggplot(juv,aes(x = LdadTL,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
  xlab('Paternal RTL (kb)') +
  ylab('Proportion male offspring') +
...
Fig2D <- ggplot(juv,aes(x = parTL,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
