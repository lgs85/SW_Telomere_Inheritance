...
FigS1B <- ggplot(FLearly,aes(x = TQ,y = LmumTLKB)) +
  geom_point(alpha=0.2) +
  xlab('Territory quality') +
  ylab('Maternal telomere length (kb)') +
  theme_lgs(addlegend = T) +
...
FigS2A <- ggplot(FL,aes(x = dadcon,y = mumcon)) +
  geom_point(alpha=0.2) +
...
FigS2B <- ggplot(FL,aes(x = mumcon,y = SexEstimate)) +
  geom_point(alpha=0.2) +
...
FigS2C <- ggplot(FL,aes(x = dadcon,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
  xlab('Paternal condition') +
  ylab('Proportion male offspring') +
...
FigS2D <- ggplot(FL,aes(x = parcon,y = SexEstimate)) +
  geom_point(alpha=0.2) +
  stat_smooth(method = 'glm',family = 'binomial',se=T) +
